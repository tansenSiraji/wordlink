...
var alpha = [], alpha1 = [], alpha2 = [], alpha3 = [],  alpha5 = [], wordList = [], wordStack = {}, wordList2 = [], obj = []
...
// console.log( ar )
  ...
  for (const item in word) {
    // console.log(word[item], 'hahah');
    word2 = word[item].split('')
  }
    // console.log(word2, 'word2222222')

    // if(word2 == obj[0]){
    //   console.log("MATCHES")
    // }
  // var word2 = word
  console.log(storeWord, "STORE WORD")
  var store = storeWord.join('')
  // store.toString()
  console.log(store)
  const found = wordList2.find(element => element == store)
  console.log(found, 'FOUND')
  if(found){
    arr = Array.from(found)
    console.log(arr, "ARRAY")

  //   var split = found.split()
  //   // var word3 = word2.split('')
  ...
  // return wordList.map((data, key) => {
    ...
    for(var i = 0; i < wordList.length; i++){
      ...
      wordStack = Object.assign({}, wordList);
      // console.log(wordStack, "wordStack")
        ...
        return arr.map((data, key) => {
          ...
          return (
            ...
            <View key = {index} style = {styles.middle} ><Text style={{ fontSize: 20, fontWeight: 'bold', justifyContent: 'center', textAlignVertical: 'center', textAlign: 'center'}}>{data}</Text></View>
            </View>
  ...
  alphabetGenerator(){
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      if (one == words[i].length) {
        ...
        wordList2.push({word: words[i]})

      }
    }

    this.setState({wordList: wordList})
    for(let [key, value] of Object.entries(wordList2)){
      obj.push({num: key, word: value})
   }

  //  for(var i = 0; i < obj.length; i++){
    // console.log(obj[i].word, "Object MAMU")
  //  }
    
    console.log("Generated Words:", wordList2)
  ...
  addLetters(alphabet){
    ...
    // console.log(obj[0].word, "MAMU ASHO")
        ...
        <View style={{
          ...
          <View style={{
            ...
            {wordList2.map((data, key) => {
              ...
              var index = key
              var word = data.split()
              return <View style = {{flexDirection: "row",}}>{this.wordBoxGenerator(word, index)}</View>
            })}
